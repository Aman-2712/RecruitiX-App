...
logger = logging.getLogger("recruitix.email")

RESEND_API_KEY = os.getenv("RESEND_API_KEY", "")
FRONTEND_URL = os.getenv("FRONTEND_URL", "http://localhost:3000").rstrip("/")
RESEND_FROM_EMAIL = os.getenv("RESEND_FROM_EMAIL", "Recruitix <onboarding@resend.dev>")
...
def send_verification_email(to_email: str, verification_token: str):
    ...
    subject = "Verify your Recruitix account"
    html_content = f"""
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px;">
        <h2 style="color: #1e3a8a; margin-bottom: 20px;">Welcome to Recruitix!</h2>
...
def send_password_reset_email(to_email: str, reset_token: str):
    ...
    subject = "Reset your Recruitix password"
    html_content = f"""
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px;">
        <h2 style="color: #1e3a8a; margin-bottom: 20px;">Password Reset Request</h2>
        <p style="font-size: 14px; color: #475569; line-height: 1.5;">
            We received a request to reset your password for your RecruitiX account. Click the button below to choose a new password. This link is valid for 1 hour.
        </p>
        <div style="margin: 30px 0; text-align: center;">
